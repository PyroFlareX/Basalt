...
std::vector<char> bs::readFile(const std::string& filePath)
{
	...
	/*	//This section turns the binary loaded into a C array
...
bs::Mesh bs::loadMeshFromObj(const std::string& filepath)
{
	...
	// Loop over shapes
	for (auto s = 0; s < shapes.size(); s += 1) 
	{
		// Loop over faces(polygon)
		size_t index_offset = 0;
		for (auto f = 0; f < shapes[s].mesh.num_face_vertices.size(); f += 1)
		{
			// Loop over vertices in the face.
			const auto fv = shapes[s].mesh.num_face_vertices[f];
			for (auto v = 0; v < fv; v += 1)
			{
				// access to vertex
